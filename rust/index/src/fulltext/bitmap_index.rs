...
impl FullTextBitmapWriter {
    ...
    pub async fn write_to_blockfiles(&self) -> Result<(), FullTextBitmapError> {
        ...
        for bucket_id in bucket_ids {
            ...
            if bitmap.is_empty() {
                ...
                self.blockfile_writer.set("", bucket_id, bitmap).await?;
            }
        }
        Ok(())
    }
    ...
    pub async fn commit(self) -> Result<FullTextBitmapFlusher, FullTextBitmapError> {
        let flusher = self.blockfile_writer.commit::<u32, RoaringBitmap>().await?;
...
pub struct FullTextBitmapReader {
    blockfile_reader: BlockfileReader<'static, u32, RoaringBitmap>,
    analyzer: WordAnalyzer,
}
...
mod tests {
    ...
    async fn commit_and_read(
        ...
        let blockfile_reader = provider
            ...
            .await
            .unwrap();
        FullTextBitmapReader::new(blockfile_reader, WordAnalyzer::new())
    }
